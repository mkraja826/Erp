...
export default async function CoursePage({ params }: PageProps) {
  ...
  const lessons = course.course_modules.flatMap((module) =>
  ...
  return (
    <main className="coursePage">
      <header className="courseTopbar">
        ...
        <span className="courseBadge">{course.module_code} · {course.difficulty}</span>
      </header>

      <section className="courseHero">
        <div>
          <span className="eyebrow">Live learning path</span>
          <h1>{course.title}</h1>
          <p>{course.description}</p>
        </div>
        <div className="courseStats">
          <div><strong>{course.course_modules.length}</strong><span>Modules</span></div>
          <div><strong>{lessons.length}</strong><span>Lessons</span></div>
          <div><strong>{totalMinutes}</strong><span>Minutes</span></div>
        </div>
      </section>

      <section className="learningRule">
        <strong>How this works</strong>
        <span>Understand → Try → Get a hint if stuck → Verify → Move forward</span>
      </section>
        ...
        {course.course_modules.map((module) => (
          <section className="courseModule" key={module.id}>
            <div className="moduleHeading">
              <span className="moduleNumber">{String(module.position).padStart(2, "0")}</span>
              <div>
                <h2>{module.title}</h2>
      ...
      <section className="workLabTeaser">
        <span className="eyebrow">After course completion</span>
        <h2>Your Work Lab unlocks next</h2>
        <p>Training guidance reduces and you start handling realistic junior SAP MM tasks like an employee.</p>
      </section>
    </main>
  );
}
