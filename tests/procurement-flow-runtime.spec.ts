...
test.describe('Procure-to-pay simulator runtime', () => {
  test('enforces master data and runs a certified procure-to-pay chain with reversals', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      const afterGrReverseFlow=await flow();const afterGrReverse=await runtime();
      const reversedBalance=Number((afterGrReverse.inventory??[]).find((row:Record<string,unknown>)=>row.material_code==='MAT-101'&&row.plant_code==='HYD1'&&row.storage_location_code==='SL01')?.quantity??0);
      ...
      return {baseline,firstBalance,secondBalance,reversedBalance,invalidMaterial,invalidPlant,invalidVendor,invalidPurchOrg,invalidStorage,pr,po,gr1,gr2,duplicatePo,overReceipt,mismatch,blockedReplacement,reverseMismatch,duplicateInvoice,matched,grReverseBlocked,reverseMatched,afterInvoiceReverse,reverseGr,afterGrReverseFlow,replacementGr,finalInvoice,closedInvoiceAttempt,afterPr,afterPo,finalFlow};
    });
    ...
    expect(result.gr1.openQuantity).toBe(40);expect(result.firstBalance).toBe(result.baseline+60);
    expect(result.gr2.openQuantity).toBe(0);expect(result.secondBalance).toBe(result.baseline+100);
    ...
    expect(result.reverseGr.status).toBe('reversed');expect(result.reverseGr.reversalMovementType).toBe('102');expect(result.reverseGr.openQuantity).toBe(40);expect(result.reverseGr.poStatus).toBe('partially_received');expect(result.reversedBalance).toBe(result.baseline+60);
