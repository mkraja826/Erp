...
export async function GET(request:Request){
  ...
  if(eligible){
    ...
    if(!existing[0]){const code=`ERP-${course.module_code}-${Date.now().toString(36).toUpperCase()}-${user.id.slice(0,4).toUpperCase()}`;await supabaseRest("certificates",{method:"POST",headers:{Prefer:"return=minimal"},body:JSON.stringify({user_id:user.id,course_id:course.id,certificate_type:"practical_competency",verification_code:code,score:overall,metadata:{courseScore,practicalScore,incidentScore,independenceScore,level:level(overall)},is_public:true})},token);}
  }
  ...
  return NextResponse.json({learner:{email:user.email??null,fullName:profile?.full_name??null,slug:profile?.public_profile_slug},course,eligible,skills:{courseScore,practicalScore,incidentScore,independenceScore,overall,level:level(overall)},evidence:{workAttempts:work.length,workPassed:work.filter(x=>x.result==="pass").length,incidentAttempts:incidents.length,incidentsResolved:incidents.filter(x=>x.result==="pass").length},certificates});
}
