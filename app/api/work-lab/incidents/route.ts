...
async function ensureIncidents(userId:string,token:string){
  ...
  const pos=docs.filter(d=>d.document_type==="PO");
  const grs=docs.filter(d=>d.document_type==="GR");
  const ivs=docs.filter(d=>d.document_type==="IV");
  ...
  for(const po of pos){
    const linked=grs.filter(gr=>String(gr.header.source_po??"")===po.document_number);
    const ordered=Number(po.items?.[0]?.quantity??0);
    const received=linked.reduce((sum,gr)=>sum+Number(gr.items?.[0]?.received_quantity??0),0);
    if(linked.length===0){
  ...
  for(const row of candidates){
    try{await supabaseRest("work_lab_incidents?on_conflict=user_id,incident_type,source_document_number",{method:"POST",headers:{Prefer:"resolution=ignore-duplicates,return=minimal"},body:JSON.stringify(row)},token);}catch{/* duplicate-safe generation */}
  }
}
...
export async function GET(request:Request){
  ...
  await ensureIncidents(user.id,token);
  const incidents=await supabaseRest<Incident[]>(`work_lab_incidents?user_id=eq.${user.id}&select=id,incident_type,title,description,priority,source_document_number,status,created_at&order=created_at.desc`,{},token);
  const attempts=await supabaseRest<Attempt[]>(`work_lab_incident_attempts?user_id=eq.${user.id}&select=id,incident_id,score,ai_help_count,result,created_at&order=created_at.desc`,{},token);
...
export async function POST(request:Request){
  const token=tokenFrom(request);if(!token)return NextResponse.json({error:"Authentication required"},{status:401});
  const user=await getSupabaseUser(token);if(!user)return NextResponse.json({error:"Invalid session"},{status:401});
  const body=await request.json() as {incidentId?:string;rootCause?:string;resolution?:string;aiHelpCount?:number};
  if(!body.incidentId)return NextResponse.json({error:"incidentId is required"},{status:400});
  const incidents=await supabaseRest<Array<Incident&{expected_resolution:Record<string,unknown>}>>(`work_lab_incidents?id=eq.${body.incidentId}&user_id=eq.${user.id}&select=id,incident_type,title,description,priority,source_document_number,status,created_at,expected_resolution&limit=1`,{},token);
  const incident=incidents[0];if(!incident)return NextResponse.json({error:"Incident not found"},{status:404});
  const root=normalizeText(body.rootCause??"");const resolution=normalizeText(body.resolution??"");
  const rootTerms=(incident.expected_resolution.root_cause_terms as string[]|undefined)??[];
  const resolutionTerms=(incident.expected_resolution.resolution_terms as string[]|undefined)??[];
  const rootScore=includesAny(root,rootTerms)?50:root.length>=20?25:0;
  const resolutionScore=includesAny(resolution,resolutionTerms)?50:resolution.length>=20?25:0;
  const score=rootScore+resolutionScore;const passed=score>=80;const result=passed?"pass":score>0?"partial":"fail";
  await supabaseRest("work_lab_incident_attempts",{method:"POST",headers:{Prefer:"return=minimal"},body:JSON.stringify({incident_id:incident.id,user_id:user.id,submitted_root_cause:body.rootCause??"",submitted_resolution:body.resolution??"",score,ai_help_count:Math.max(0,body.aiHelpCount??0),result})},token);
  if(passed)await supabaseRest(`work_lab_incidents?id=eq.${incident.id}&user_id=eq.${user.id}`,{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify({status:"resolved",resolved_at:new Date().toISOString()})},token);
