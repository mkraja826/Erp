...
export async function POST(request:Request){
  ...
  try{
    if(body.action==="post_source"){
      ...
      const currency=String(source.header.currency??"INR");let items:Array<Record<string,unknown>>=[];const postingDate=String(source.header.posting_date??new Date().toISOString().slice(0,10));let referenceType="";
    ...
    if(body.action==="reverse_fi"){
      ...
      const sourceType=String(fi.header.source_type??"");const clearingStatus=String(fi.header.clearing_status??"open");
      if(sourceType==="IV"&&clearingStatus==="cleared")return NextResponse.json({error:`Invoice FI ${fi.document_number} is cleared. Reverse the vendor payment before reversing the FI document.`},{status:409});
      if(sourceType==="BILL"&&clearingStatus==="cleared")return NextResponse.json({error:`Customer receivable FI ${fi.document_number} is cleared. Reverse the customer payment before reversing the FI document.`},{status:409});
      const reversalItems=(fi.items??[]).map(x=>({...x,debit:amount(x.credit),credit:amount(x.debit),text:`Reversal · ${String(x.text??"")}`}));const t=totals(reversalItems);const reversalNumber=docNumber();await supabaseRest("erp_documents",{method:"POST",headers:{Prefer:"return=minimal"},body:JSON.stringify({user_id:user.id,document_type:"FI",document_number:reversalNumber,status:"posted",header:{source_document:fi.document_number,source_type:"FI_REVERSAL",reversal_of:fi.document_number,posting_date:new Date().toISOString().slice(0,10),currency:fi.header.currency??"INR",total_debit:t.debit,total_credit:t.credit,balanced:true,accounting_phase:fi.header.accounting_phase??"FI"},items:reversalItems})},token);await supabaseRest(`erp_documents?id=eq.${fi.id}`,{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify({status:"reversed",header:{...fi.header,reversed_by:reversalNumber,reversal_date:new Date().toISOString().slice(0,10)}})},token);
