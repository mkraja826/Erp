...
type LockState={message:string;requirement:string;progressPercent:number;enrollmentStatus:string;actionUrl:string;actionLabel:string};
...
export default function WorkplaceInboxPage(){
 ...
 useEffect(()=>{void load();},[]);
 async function load(){
  setLoading(true);setLock(null);
  try{
   const r=await authenticatedFetch("/api/workplace-inbox");
   const p=await r.json();
   if(!r.ok){
    if(r.status===403&&p.code==="SAP_MM_REQUIRED")setLock({message:p.message??"Complete SAP MM Level 1 to unlock your simulated workplace inbox.",requirement:p.requirement??"SAP MM Level 1 · 100% complete",progressPercent:Number(p.progressPercent??0),enrollmentStatus:String(p.enrollmentStatus??"not_started"),actionUrl:p.actionUrl??"/courses/sap-mm-level-1",actionLabel:p.actionLabel??"Continue SAP MM"});
    else setMessage(p.error??"Unable to load workplace inbox.");
    setData(null);return;
   }
   setData(p as Data);setMessage("");setLock(null);
