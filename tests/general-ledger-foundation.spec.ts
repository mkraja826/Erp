...
test.describe('General ledger foundation',()=>{
  test('posts balanced journals and produces trial balance plus statements',async({page},testInfo)=>{
    ...
    const result=await page.evaluate(async()=>{
      ...
      const before=await post('financial_statements',{fiscal_period:period});
      ...
      return{period,before,bad,unknown,journal,trial,statements};
    });
    ...
    expect(result.statements.balanceSheet.assets-result.before.balanceSheet.assets).toBe(1000);
    expect(result.statements.balanceSheet.liabilitiesAndEquity-result.before.balanceSheet.liabilitiesAndEquity).toBe(1000);
  });
});
