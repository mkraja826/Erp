...
export async function POST(request:Request){
  ...
  try{
    ...
    if(body.action==="reverse_fi"){
      ...
      if(String(fi.header.source_type??"")==="IV"&&String(fi.header.clearing_status??"open")==="cleared")return NextResponse.json({error:`Invoice FI ${fi.document_number} is cleared. Reverse the vendor payment before reversing the FI document.`},{status:409});
