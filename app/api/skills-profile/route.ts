...
function level(score:number){return score>=90?"Advanced Beginner":score>=80?"Job Ready - Foundation":score>=65?"Developing":"Beginner";}
...
export async function GET(request:Request){
  ...
  const courseScore=Math.round(Number(enrollment?.progress_percent??0));const practicalScore=average(work.map(x=>x.score));const incidentScore=average(incidents.map(x=>x.score));const independenceScore=independence([...work,...incidents]);
  const overall=Math.round(courseScore*.35+practicalScore*.3+incidentScore*.2+independenceScore*.15);
  const currentLevel=level(overall);
  ...
  if(eligible){
    const existing=await supabaseRest<Certificate[]>(`certificates?user_id=eq.${user.id}&course_id=eq.${course.id}&certificate_type=eq.practical_competency&select=id,certificate_type,verification_code,score,issued_at,metadata,is_public&limit=1`,{},token);
    if(!existing[0]){const code=`ERP-${course.module_code}-${Date.now().toString(36).toUpperCase()}-${user.id.slice(0,4).toUpperCase()}`;await supabaseRest("certificates",{method:"POST",headers:{Prefer:"return=minimal"},body:JSON.stringify({user_id:user.id,course_id:course.id,certificate_type:"practical_competency",verification_code:code,score:overall,metadata:{courseScore,practicalScore,incidentScore,independenceScore,level:currentLevel},is_public:true})},token);}
  }
  const certificates=await supabaseRest<Certificate[]>(`certificates?user_id=eq.${user.id}&course_id=eq.${course.id}&select=id,certificate_type,verification_code,score,issued_at,metadata,is_public&order=issued_at.desc`,{},token);
  const publicCertificate=certificates.find(c=>c.is_public);
  if(publicCertificate&&profile?.public_profile_slug){
    await supabaseRest("public_skill_snapshots?on_conflict=profile_slug",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify({user_id:user.id,profile_slug:profile.public_profile_slug,display_name:profile.full_name||"ERP Edu Learner",course_title:course.title,module_code:course.module_code,verification_code:publicCertificate.verification_code,certificate_type:publicCertificate.certificate_type,score:publicCertificate.score,issued_at:publicCertificate.issued_at,skills:publicCertificate.metadata,updated_at:new Date().toISOString()})},token);
  }
  return NextResponse.json({learner:{email:user.email??null,fullName:profile?.full_name??null,slug:profile?.public_profile_slug},course,eligible,skills:{courseScore,practicalScore,incidentScore,independenceScore,overall,level:currentLevel},evidence:{workAttempts:work.length,workPassed:work.filter(x=>x.result==="pass").length,incidentAttempts:incidents.length,incidentsResolved:incidents.filter(x=>x.result==="pass").length},certificates});
}
