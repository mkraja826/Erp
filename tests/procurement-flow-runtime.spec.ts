...
test.describe('Procure-to-pay simulator runtime', () => {
  test('enforces master data and runs a certified procure-to-pay chain', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      return {baseline,firstBalance,secondBalance,invalidMaterial,invalidPlant,invalidVendor,invalidPurchOrg,invalidStorage,pr,po,gr1,gr2,duplicatePo,overReceipt,mismatch,duplicateInvoice,matched,closedInvoiceAttempt,afterPr,afterPo,afterFirstReceiptFlow,afterSecondReceiptFlow,finalFlow};
    });
